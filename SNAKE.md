...
игра заканчивается. Как таковых критериев победы у данной игры нет. Есть только очки которые равны количеству съеденной еды.
...
на который засыпает второй поток. Давайте сделаем так, до длины 10 T = 600 мс, а при длине 30 и больше T = 300 мс. Между длиной 10 и 30 T линейно уменьшается.
...
ее двигаться вправо не должна ничего менять. Хвост змеи проделывает в точности тот же путь, который прошла ее голова.

Кроме данных рекомендаций можете добавлять в игровой процесс что угодно. Размер поля, наличие стен, наличие оппонента (другой змеи с AI, которая будет поедать еду) и т.д.
...
Небольшая подсказка. Чтобы при отрисовки нового состояния игры не возникало морганий, не делайте Console.Clear() на каждой итерации.
...
1. [0-4] Оформление и настройка проекта. Ведение коммитов в git.
